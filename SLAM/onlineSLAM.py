...
def online_slam(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    # set the dimension of filter
    dim = 2 * (1 + num_landmarks)

    # make the constraint information matrix and v ector
    Omega = matrix()
    Omega.zero(dim, dim)
    Omega.value[0][0] = 1.0
    Omega.value[1][1] = 1.0
    ...
    for k in range(len(data)):
        ...
        for i in range(len(measurement)):
            ...
            # update the information matrix/vector based on thr measurement
            for b in range(2):
        ...
        # expand the information matrix and vector by one ne position

        list1  = [0, 1] + range(4, dim+2)
        Omega = Omega.expand(dim+2, dim+2, list1, list1)
        Xi    = Xi.expand(dim+2, list1, [0])
