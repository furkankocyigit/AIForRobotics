    ...
    while change >= tolerance:
        ...
        for i in range(1, len(path)):
            for j in range(len(path[0])):
                aux = newpath[i][j]
                newpath[i][j] += weight_data * (path[i][j] - newpath[i][j]) + weight_smooth * (newpath[(i-1)%len(path)][j] + newpath[(i+1)%len(path)][j] - 2.0 * newpath[i % len(path)][j])
...
plotRealAnswer1 = np.array(answer1)
plotAnswer = np.array(smooth(testpath1))
plt.plot(plotRealAnswer1[:,0], plotRealAnswer1[:,1])
plt.show()

plotMyAnswer1 = np.array(smooth(testpath1))
plt.plot(plotMyAnswer1[:, 0]), plotMyAnswer1[:, 1]
plt.show()
